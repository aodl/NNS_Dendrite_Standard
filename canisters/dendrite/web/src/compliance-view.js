...
export function renderSnapshot(root, snapshot, nowSeconds = BigInt(Math.floor(Date.now() / 1000))) {
  ...
  for (const rule of snapshot.rules) { const tr = document.createElement("tr"); for (const value of [rule.rule_id, variantName(rule.status), rule.summary, rule.observed[0] ?? "—", rule.expected[0] ?? "—", `${rule.source.method} @ ${rule.source.observed_at_seconds}`]) tr.append(element("td", value)); rules.append(tr); } root.append(rules);
